...
async function customFunctionsExamples() {
  ...
  jexl.addFunction('average', (arr: number[]) => 
    arr.reduce((sum, val) => sum + val, 0) / arr.length
  )
  console.log('Average salary:', await jexl.eval('average(users | map("salary"))', context))
  
  jexl.addFunction('factorial', (n: number): number => 
    n <= 1 ? 1 : n * jexl.grammar.functions.factorial(n - 1)
  )
  ...
  jexl.addFunction('wordCount', (str: string) =>
  ...
  jexl.addFunction('randomString', (length: number = 8) => {
  ...
  jexl.addFunction('formatDate', (dateStr: string, format: string = 'MM/DD/YYYY') => {
    const date = new Date(dateStr)
  ...
  jexl.addFunction('isInRange', (value: number, min: number, max: number) => 
    value >= min && value <= max
  )
  ...
  jexl.addFunction('calculateTax', (amount: number, rate: number = 0.08) => 
    Math.round(amount * rate * 100) / 100
  )
  console.log('Calculate tax:', await jexl.eval('calculateTax(1000, 0.085)'))
  
  jexl.addFunction('applyDiscount', (price: number, discountPercent: number) => 
  ...
  console.log('User summary:',
    await jexl.eval('mapInitials(users) | join(", ")', context))
  ...
  console.log('Engineering team average:',
    await jexl.eval('average(filterByDept(users, "Engineering"))', context))
  ...
  console.log('Product availability:',
    await jexl.eval('getAvailableProducts(products) | join(", ")', context))
  console.log()
}
