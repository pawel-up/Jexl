...
async function advancedFeaturesExamples() {
  ...
  jexl.addFunction('evaluatePerformance', (users: Record<string, unknown>[]) => 
    users.map(user => ({
      name: user.name,
      grade: (user.performance as any).rating >= 4.5 ? "Excellent" : 
             (user.performance as any).rating >= 4.0 ? "Good" : "Needs Improvement"
    })))
  ...
  jexl.addTransform('flatten', (obj: Record<string, unknown>, separator: string = '.') => {
  ...
  console.log('Group users by department:', 
    await jexl.eval('users | groupBy("department")', context))

  console.log('Flatten user data:', 
    await jexl.eval('users[0] | flatten', context))
  ...
  console.log('Complex aggregation:', 
    await jexl.eval('departmentHours(users) | groupBy("department")', context))
  console.log()
  ...
  jexl.addFunction('highPerformersWithJS', (users: Record<string, unknown>[]) => 
    users.filter(user => 
      ...
      (user.skills as string[]).includes('JavaScript')
    ).map(user => user.name))

  console.log('High performers with specific skills:',
    await jexl.eval('highPerformersWithJS(users)', context))
  ...
  console.log('All project statuses:',
    await jexl.eval('getAllProjectStatuses(users)', context))
  ...
  console.log('Department salary statistics:',
    await jexl.eval('departmentStats(users)', context))
  console.log()
  ...
  jexl.addFunction('allocateBudget', (totalBudget: number, departments: string[], users: Record<string, unknown>[]) => {
    ...
    return departments.map(dept => {
      ...
      const avgSalary = employeeCount > 0 ? 
        deptUsers.reduce((sum, user) => sum + (user.salary as number), 0) / employeeCount : 
        0
